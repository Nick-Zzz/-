...
{ 	 	
	GPIO_InitTypeDef GPIO_InitStructure;
 	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA, ENABLE); //使能PORTB,C时钟和AFIO时钟

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0|GPIO_Pin_1|GPIO_Pin_2;	   ///PORTC6~10复用推挽输出
	...
	LCD_WR_DATA8(0x1A);
	LCD_WR_REG(0x16); //MX, MY, RGB mode//将36H改成16H，将屏幕显示180度旋转
	LCD_WR_DATA8(0xC0);
...
{  
	unsigned char i,j,k;
	const unsigned char *temp=zhishu;    
	temp+=index*32;	
	for(j=0;j<16;j++)
	{
		LCD_SetCursor(x,y+j);
...
{  							  
	...
	if(!mode) //非叠加方式
	{
	    ...
	    {   
			...
			else temp=asc2_1608x[num][t];		 //调用1608字体 
	        for(t1=0;t1<8;t1++)
			{			    
...
void showBEIJING(u16 x,u16 y) //显示背景128*160图片
{  
	u16 i,j,k;
	u16 da;
	k=0;
